...
public class ToaTauDF {
    ...
    public static void generateSampleData(EntityManager em) {
        ...
        try {
            ...
            List<Tau> tauList = em.createQuery("SELECT t FROM Tau t", Tau.class).getResultList();
            if (tauList.isEmpty()) {
            ...
            for (int i = 1; i <= 20; i++) { // Tạo 20 toa tàu
                ...
                toaTau.setMaToa(i); // Mã toa là số nguyên tăng dần
                ...
                // Liên kết với một Tau ngẫu nhiên
                Tau randomTau = tauList.get(faker.number().numberBetween(0, tauList.size()));
                toaTau.setTau(randomTau);
